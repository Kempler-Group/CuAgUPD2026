...
# plotColors = ["#000000",  "#640d31",  "#b9095a",  "#fd2084", "#ff65a0", "#ff8eb6", "#ffaeca", "#ffcbdc", "#ffe5ed", "#ffffff"] #for coverage vs potential plot
...
def run_files(frames, plotColors, legend, scan_rates):  # Add scan_rates as a parameter
    ...
    plt.subplots_adjust( top=0.965, bottom=0.135, left=0.195, right=0.99, hspace=0.2, wspace=0.2)

    plt.xlabel('$\mathit{E}$ (V vs $\mathit{E}_\mathit{rev}$)')
    ...
    # plt.axvline(0.25, color = "lightgrey", linestyle = "dashed")
    # plt.axhline(0.05, color = "grey", linestyle = "dashed")
...
def get_K_app(pca_E, P_ks, cl):  # Pass Coverage and cl as arguments
    ...
    # a = C**(1-alpha) * (1 - C)**alpha #(C*(1 - C))**alpha if alpha = 0.5
    #a = C**(1-alpha) * (1 - C)**alpha #(C*(1 - C))**alpha if alpha = 0.5
    ...
    def func(x, k):
        return (k) * (np.exp(-(con2 * x)) - np.exp((con * x)))#eqn from suntivisch paper
    ...
    s = "$\mathregular{s^{-1}}$"
    r_o = '$\mathit{r}_\mathrm{0}$'
    r_app_str = f"{r_o} = {k_app:.2g} {s}"  # Formatted string for k_app
